			...
			value: function request(req, res) {
				...
				fs.lstat(lpath, function (e, stats) {
					if (e) {
						...
						_this.handle(req, res, lpath, req.parsed.href, false, stats);
						...
						_this.handle(req, res, lpath, req.parsed.href, true);
					} else {
						count = 0;
						...
						array.iterate(_this.config.index, function (i) {
							...
							fs.lstat(npath, function (e, stats) {
								if (!e && !handled) {
									...
									_this.handle(req, res, npath, (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, stats);
    ...
    */
			...
			value: function start(cfg, err) {
				...
				if (!this.config.headers.server) {
					this.config.headers.server = "turtle.io/3.2.15";
