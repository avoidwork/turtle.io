	...
	if (regex.body.test(req.method)) {
		...
		req.on("data", function (data) {
			...
			if (server.config.maxBytes > 0 && Buffer.byteLength(payload) > server.config.maxBytes) {
				req.invalid = true;
				next(new Error(_this.codes.REQ_TOO_LARGE));
			}
		});
	...
	_createClass(TurtleIO, [{
		...
		value: function allowed(method, uri, host, override) {
			...
			result = this.routes(uri, host, method, override).filter(function (i) {
				return _this2.config.noaction[i.hash || _this2.hash(i)] === undefined;
			});
		...
		value: function allows(uri, host, override) {
			...
			if (override || !result) {
				result = VERBS.filter(function (i) {
					return _this3.allowed(i, uri, host, override);
				});
   ...
   */
		...
		value: function decorate(req, res) {
			...
			res.redirect = function (target) {
				return _this5.respond.call(_this5, req, res, _this5.messages.NO_CONTENT, _this5.codes.FOUND, { location: target });
			};

			res.respond = function (arg, status, headers) {
				return _this5.respond.call(_this5, req, res, arg, status, headers);
			};

			res.error = function (status, arg) {
				return _this5.error.call(_this5, req, res, status, arg);
			};
		}
   ...
   */
		...
		value: function headers(req) {
			...
			if (status !== this.codes.NOT_MODIFIED && status >= this.codes.MULTIPLE_CHOICE && status < this.codes.BAD_REQUEST) {
				...
				lheaders = merge(clone(this.config.headers), rHeaders);
   ...
   */
    ...
    */
			var handle = function handle(req, res, headers, status, arg) {
				...
				headers.server = _this9.config.headers.server;

				if (status >= _this9.codes.BAD_REQUEST) {
					_this9.error(req, res, status, arg).then(function (argz) {
						deferred.resolve(argz);
					});
				} else {
					// Determining if the response will be cached
					if (get && (status === _this9.codes.SUCCESS || status === _this9.codes.NOT_MODIFIED) && !regex.nocache.test(headers["cache-control"]) && !regex["private"].test(headers["cache-control"])) {
						...
						if (stale > 0) {
							setTimeout(function () {
								_this9.unregister(uri);
							}, stale * 1000);
						}
					}

					if (status !== _this9.codes.NOT_MODIFIED) {
						rewrite = regex.rewrite.test((headers["content-type"] || "").replace(regex.nval, ""));

						// Setting headers
						if (get && status === _this9.codes.SUCCESS) {
							letag = headers.etag || "\"" + _this9.etag(uri, headers["content-length"] || 0, headers["last-modified"] || 0, _this9.encode(arg)) + "\"";
						...
						if (get && req.headers["if-none-match"] === letag) {
							cached = _this9.etags.get(uri);

							if (cached) {
								headers.age = parseInt(new Date().getTime() / 1000 - cached.value.timestamp, 10);
							}

							_this9.respond(req, res, _this9.messages.NO_CONTENT, _this9.codes.NOT_MODIFIED, headers).then(function (argz) {
								deferred.resolve(argz);
							}, function (e) {
								deferred.reject(e);
							});
							...
							if (regex.head.test(req.method.toLowerCase())) {
								larg = _this9.messages.NO_CONTENT;
							} else if (rewrite) {
							...
							_this9.respond(req, res, larg, status, headers).then(function (argz) {
								deferred.resolve(argz);
							}, function (e) {
								deferred.reject(e);
							});
						...
						_this9.respond(req, res, arg, status, headers).then(function (argz) {
							deferred.resolve(argz);
						}, function (e) {
							deferred.reject(e);
						});
    ...
    */
			var wrapper = function wrapper(req, res) {
				...
				headerz["x-forwarded-server"] = _this9.config.headers.server;
				...
				proxyReq.on("error", function (e) {
					_this9.error(req, res, _this9.codes[regex.refused.test(e.message) ? "SERVER_UNAVAILABLE" : "SERVER_ERROR"], e.message);
				});
			...
			array.each(VERBS, function (i) {
				if (route === "/") {
					_this9[i]("/.*", wrapper, host);
				} else {
					_this9[i](route, wrapper, host);
					_this9[i](route + "/.*", wrapper, host);
				}
			});
   ...
   */
		...
		value: function redirect(route, uri, host) {
			...
			this.get(route, function (req, res) {
				...
				_this10.respond(req, res, _this10.messages.NO_CONTENT, _this10.codes[permanent ? "MOVED" : "REDIRECT"], {
   ...
   */
		...
		value: function request(req, res) {
			...
			var end = function end() {
				timer.stop();
				_this11.signal("request", function () {
			...
			if (req.headers.expect) {
				...
				fs.lstat(lpath, function (e, stats) {
					if (e) {
						...
						deferred.reject(new Error(_this11.codes.NOT_FOUND));
					} else if (!stats.isDirectory()) {
						end();
						_this11.handle(req, res, lpath, req.parsed.href, false, stats).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						_this11.respond(req, res, _this11.messages.NO_CONTENT, _this11.codes.REDIRECT, { "location": (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + req.parsed.search }).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						_this11.handle(req, res, lpath, req.parsed.href, true).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						count = 0;
						nth = _this11.config.index.length;

						array.each(_this11.config.index, function (i) {
							...
							fs.lstat(npath, function (err, lstats) {
								if (!err && !handled) {
									...
									_this11.handle(req, res, npath, (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, lstats).then(function (arg) {
										deferred.resolve(arg);
									}, function (errz) {
   ...
   */
		...
		value: function respond(req, res, body) {
			...
			var finalize = function finalize() {
				...
				if (regex.get_only.test(req.method) && (lstatus === _this12.codes.SUCCESS || lstatus === _this12.codes.NOT_MODIFIED)) {
					// Updating cache
					if (!regex.nocache.test(lheaders["cache-control"]) && !regex["private"].test(lheaders["cache-control"])) {
						cached = _this12.etags.get(req.parsed.href);

						if (!cached) {
							if (lheaders.etag === undefined) {
								lheaders.etag = "\"" + _this12.etag(req.parsed.href, lbody.length || 0, lheaders["last-modified"] || 0, lbody || 0) + "\"";
							}
							...
							_this12.register(req.parsed.href, {
					...
					if (req.path) {
						_this12.watch(req.parsed.href, req.path);
					}
				} else if (lstatus === _this12.codes.NOT_FOUND) {
			...
			if (ua && (lstatus === this.codes.SUCCESS || lstatus === this.codes.PARTIAL_CONTENT) && lbody !== this.messages.NO_CONTENT && this.config.compress && (type = this.compression(ua, encoding, lheaders["content-type"])) && type !== null) {
				...
				fs.createReadStream(lbody, options).on("error", function () {
					deferred.reject(new Error(_this12.codes.SERVER_ERROR));
				}).on("close", function () {
					deferred.resolve(true);
				}).pipe(res);
			} else {
			...
			process.nextTick(function () {
				_this12.log(_this12.prep(req, res, lheaders), "info");
			});
		...
		value: function route(req, res) {
			...
			var last = function last(err) {
				...
				if (!err) {
					...
					} else if (_this13.allowed("get", req.parsed.pathname, req.vhost)) {
						deferred.reject(new Error(_this13.codes.NOT_ALLOWED));
					} else {
						deferred.reject(new Error(_this13.codes.NOT_FOUND));
					}
				} else {
					status = res.statusCode >= _this13.codes.BAD_REQUEST ? res.statusCode : !isNaN(err.message) ? err.message : _this13.codes[(err.message || err).toUpperCase()] || _this13.codes.SERVER_ERROR;
			...
			var next = function next(err) {
				...
				} else if (!res._header && _this13.config.catchAll) {
					last(err);
				} else if (res._header) {
					deferred.resolve([req, res]);
				}
   ...
   */
		...
		value: function start(cfg, err) {
			...
			iterate(headers, function (value, key) {
				_this14.config.headers[key.toLowerCase()] = value;
			});
			...
			fs.readdir(pages, function (e, files) {
				if (e) {
					_this14.log(new Error("[client 0.0.0.0] " + e.message), "error");
				} else if (array.keys(_this14.config).length > 0) {
					var next = function next(req, res) {
						_this14.route(req, res).then(function (arg) {
							return arg;
						}, function (errz) {
							var body = undefined,
							    status = undefined;
							...
							if (isNaN(errz.message)) {
								body = errz;
								status = new Error(_this14.codes.SERVER_ERROR);
							} else {
								body = errz.extended;
								status = Number(errz.message);
							}

							return _this14.error(req, res, status, body);
						});
					};

					array.each(files, function (i) {
						_this14.pages.all[i.replace(regex.next, "")] = fs.readFileSync(path.join(pages, i), "utf8");
					});

					// Starting server
					if (_this14.server === null) {
						if (_this14.config.ssl.cert !== null && _this14.config.ssl.key !== null) {
							// POODLE
							_this14.config.secureProtocol = "SSLv23_method";
							_this14.config.secureOptions = constants.SSL_OP_NO_SSLv3 | constants.SSL_OP_NO_SSLv2;

							// Reading files
							_this14.config.ssl.cert = fs.readFileSync(_this14.config.ssl.cert);
							_this14.config.ssl.key = fs.readFileSync(_this14.config.ssl.key);

							// Starting server
							_this14.server = https.createServer(merge(_this14.config.ssl, {
								port: _this14.config.port,
								host: _this14.config.address,
								secureProtocol: _this14.config.secureProtocol,
								secureOptions: _this14.config.secureOptions
							}), next).listen(_this14.config.port, _this14.config.address);
						} else {
							_this14.server = http.createServer(next).listen(_this14.config.port, _this14.config.address);
						}
					} else {
						_this14.server.listen(_this14.config.port, _this14.config.address);
					}

					// Dropping process
					if (_this14.config.uid && !isNaN(_this14.config.uid)) {
						process.setuid(_this14.config.uid);
					}

					_this14.log("Started " + _this14.config.id + " on port " + _this14.config.port, "debug");
				}
			});
   ...
   */
    ...
    */
			var cleanup = function cleanup() {
				watcher.close();
				_this17.unregister(uri);
				delete _this17.watching[fpath];
			};
					...
					if (regex.rename.test(ev)) {
						...
						fs.lstat(fpath, function (e, stat) {
							...
							if (e) {
								_this17.log(e);
								cleanup();
							} else if (_this17.etags.cache[uri]) {
								value = _this17.etags.cache[uri].value;
								value.etag = _this17.etag(uri, stat.size, stat.mtime).toString();
								value.headers.etag = "\"" + value.etag + "\"";
								value.timestamp = parseInt(new Date().getTime() / 1000, 10);
								_this17.register(uri, value, true);
							} else {
								cleanup();
							}
						});
		...
		value: function write(req, res, fpath) {
			...
			if (!put && regex.end_slash.test(req.url)) {
				...
				fs.lstat(fpath, function (e, stat) {
					...
					if (e) {
						deferred.reject(new Error(_this18.codes.NOT_FOUND));
					} else {
						letag = "\"" + _this18.etag(req.parsed.href, stat.size, stat.mtime) + "\"";

						if (!req.headers.hasOwnProperty("etag") || req.headers.etag === letag) {
							fs.writeFile(fpath, body, function (err) {
								if (err) {
									deferred.reject(new Error(_this18.codes.SERVER_ERROR));
								} else {
									status = _this18.codes[put ? "NO_CONTENT" : "CREATED"];
									deferred.resolve(_this18.respond(req, res, _this18.page(status, _this18.hostname(req)), status, { allow: allow }, false));
								}
							});
						} else if (req.headers.etag !== letag) {
							deferred.resolve(_this18.respond(req, res, _this18.messages.NO_CONTENT, _this18.codes.FAILED, {}, false));
						}
					}
				});
