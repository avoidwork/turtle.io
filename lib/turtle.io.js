   ...
   */
		value: function decorate(req, res) {
			...
			res.redirect = function (uri) {
				return _this5.respond(req, res, _this5.messages.NO_CONTENT, _this5.codes.FOUND, { location: uri }).then(function () {
					_this5.log(_this5.prep(req, res, res._headers || {}), "info");
				});
			};

			res.respond = function (arg, status, headers) {
				return _this5.respond(req, res, arg, status, headers).then(function () {
					_this5.log(_this5.prep(req, res, res._headers || {}), "info");
				});
			};

			res.error = function (status, arg) {
				return _this5.error(req, res, status, arg).then(function () {
					_this5.log(_this5.prep(req, res, res._headers || {}), "info");
				});
			};
   ...
   */
		value: function start(cfg, err) {
			...
			if (!this.config.headers.server) {
				this.config.headers.server = "turtle.io/4.0.10";
