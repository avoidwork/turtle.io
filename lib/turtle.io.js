...
var TurtleIO = (function () {
   ...
   */
		value: function decorate(req, res) {
			...
			res.redirect = function (uri) {
				_this5.respond(req, res, _this5.messages.NO_CONTENT, _this5.codes.FOUND, { location: uri });
			};
   ...
   */
		value: function error(req, res, status, msg) {
			...
			this.signal("error", function () {
				return [req.vhost, req.parsed.path, status, msg || kdx ? array.cast(_this6.messages)[kdx] : "Unknown error", timer.diff()];
			});
    ...
    */
			var handle = function handle(req, res, headers, status, arg) {
				...
				if (status >= _this10.codes.BAD_REQUEST) {
					_this10.error(req, res, status, arg).then(function (arg) {
						deferred.resolve(arg);
					});
				} else {
					...
					if (status !== _this10.codes.NOT_MODIFIED) {
						...
						if (get && status === _this10.codes.SUCCESS) {
							etag = headers.etag || "\"" + _this10.etag(url, headers["content-length"] || 0, headers["last-modified"] || 0, _this10.encode(arg)) + "\"";
						...
						if (get && req.headers["if-none-match"] === etag) {
							cached = _this10.etags.get(url);

							if (cached) {
								headers.age = parseInt(new Date().getTime() / 1000 - cached.value.timestamp, 10);
							}

							_this10.respond(req, res, _this10.messages.NO_CONTENT, _this10.codes.NOT_MODIFIED, headers).then(function (arg) {
								deferred.resolve(arg);
							}, function (e) {
								deferred.reject(e);
							});
							...
							if (regex.head.test(req.method.toLowerCase())) {
								arg = _this10.messages.NO_CONTENT;
							} else if (rewrite) {
							...
							_this10.respond(req, res, arg, status, headers).then(function (arg) {
								deferred.resolve(arg);
							}, function (e) {
								deferred.reject(e);
							});
   ...
   */
		value: function redirect(route, url, host) {
			var _this11 = this;
					...
					array.each(_this12.config.index, function (i) {
						...
						fs.lstat(npath, function (e, stats) {
							if (!e && !handled) {
								...
								_this12.handle(req, res, npath, (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, stats).then(function (arg) {
									deferred.resolve(arg);
								}, function (e) {
									deferred.reject(e);
								});
   ...
   * @return {Object}          TurtleIO instance
   */
		value: function respond(req, res, body, _x4, headers) {
			...
			var finalize = function finalize() {
				...
				if (regex.get_only.test(req.method) && (status === _this13.codes.SUCCESS || status === _this13.codes.NOT_MODIFIED)) {
					// Updating cache
					if (!regex.nocache.test(headers["cache-control"]) && !regex["private"].test(headers["cache-control"])) {
						...
						if (!cached) {
							if (headers.etag === undefined) {
								headers.etag = "\"" + _this13.etag(req.parsed.href, body.length || 0, headers["last-modified"] || 0, body || 0) + "\"";
							}
				...
				} else if (status === _this13.codes.NOT_FOUND) {
   ...
   */
		value: function start(cfg, err) {
			...
			if (!this.config.headers.server) {
				this.config.headers.server = "turtle.io/4.0.4";
				...
				if (e) {
					...
					var next = function next(req, res) {
						_this15.pipeline(req, res).then(function (arg) {
							...
							_this15.log(_this15.prep(req, res, res._headers || {}), "info");
						});
					};
   ...
   */
    ...
    */
			var cleanup = function cleanup() {
				watcher.close();
				_this18.unregister(url);
				delete _this18.watching[path];
			};
					...
					if (regex.rename.test(ev)) {
						...
						fs.lstat(path, function (e, stat) {
							...
							if (e) {
								_this18.log(e);
								cleanup();
							} else if (_this18.etags.cache[url]) {
								value = _this18.etags.cache[url].value;
								value.etag = _this18.etag(url, stat.size, stat.mtime);
								value.timestamp = parseInt(new Date().getTime() / 1000, 10);
								_this18.register(url, value, true);
							} else {
								cleanup();
							}
						});
