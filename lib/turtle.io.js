...
var TurtleIO = (function () {
  ...
  */
		...
		value: function decorate(req, res) {
			...
			if (!this.allowed("get", req.parsed.pathname, req.vhost)) {
				this.get(req.parsed.pathname, function (req, res, next) {
					_this5.request(req, res).then(next, next);
				}, req.vhost);
			...
			res.redirect = function (uri) {
				_this5.respond(req, res, MESSAGES.NO_CONTENT, CODES.FOUND, { location: uri });
			};

			res.respond = function (arg, status, headers) {
				_this5.respond(req, res, arg, status, headers);
			};

			res.error = function (status, arg) {
				_this5.error(req, res, status, arg);
			};
				...
				if (regex.get.test(method)) {
					...
					fs.unlink(path, function (e) {
						if (e) {
							...
							_this6.respond(req, res, MESSAGES.NO_CONTENT, CODES.NO_CONTENT, {}).then(function (arg) {
								deferred.resolve(arg);
							}, function (e) {
								deferred.reject(e);
							});
   ...
   */
		value: function pipeline(req, res) {
			...
			return this.decorate(req, res).then(function (args) {
				return _this8.connect(args);
			}).then(function (args) {
				return _this8.route(args);
			});
		}
	}, {
		key: "prep",
   ...
   */
    ...
    */
			var handle = function handle(req, res, arg, xhr) {
				...
				resHeaders.server = _this9.config.headers.server;

				// Something went wrong
				if (xhr.status < CODES.CONTINUE) {
					_this9.error(req, res, CODES.BAD_GATEWAY);
				} else if (xhr.status >= CODES.SERVER_ERROR) {
					_this9.error(req, res, xhr.status);
				} else {
					...
					if (xhr.status !== CODES.NOT_MODIFIED) {
						...
						if (get && xhr.status === CODES.SUCCESS) {
							etag = resHeaders.etag || "\"" + _this9.etag(url, resHeaders["content-length"] || 0, resHeaders["last-modified"] || 0, _this9.encode(arg)) + "\"";
						...
						if (get && req.headers["if-none-match"] === etag) {
							cached = _this9.etags.get(url);

							if (cached) {
								resHeaders.age = parseInt(new Date().getTime() / 1000 - cached.value.timestamp, 10);
							}

							_this9.respond(req, res, MESSAGES.NO_CONTENT, CODES.NOT_MODIFIED, resHeaders);
						} else {
							...
							_this9.respond(req, res, arg, xhr.status, resHeaders);
						}
					} else {
						_this9.respond(req, res, arg, xhr.status, resHeaders);
					}
			...
			var wrapper = function wrapper(req, res) {
				...
				if (streamd) {
					...
					proxyReq.on("error", function (e) {
						_this9.error(req, res, regex.refused.test(e.message) ? CODES.SERVER_UNAVAILABLE : CODES.SERVER_ERROR);
					});
   ...
   */
		value: function redirect(route, url, host, permanent) {
			...
			this.get(route, function (req, res) {
				...
				_this10.respond(req, res, MESSAGES.NO_CONTENT, code, {
		...
		value: function request(req, res) {
			...
			fs.lstat(lpath, function (e, stats) {
				if (e) {
					...
					_this11.respond(req, res, MESSAGES.NO_CONTENT, CODES.REDIRECT, { "Location": (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + req.parsed.search }).then(function (arg) {
						deferred.resolve(arg);
					}, function (e) {
						deferred.reject(e);
					});
					...
					array.each(_this11.config.index, function (i) {
						...
						fs.lstat(npath, function (e, stats) {
							if (!e && !handled) {
								...
								_this11.handle(req, res, npath, (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, stats).then(function (arg) {
									deferred.resolve(arg);
								}, function (e) {
									deferred.reject(e);
								});
   ...
   * @return {Object}          TurtleIO instance
   */
		value: function respond(req, res, body, _x2, headers) {
						...
						if (regex.get_only.test(req.method) && status === CODES.SUCCESS) {
							...
							if (!regex.nocache.test(headers["cache-control"]) && !regex["private"].test(headers["cache-control"])) {
								if (headers.etag === undefined) {
									headers.etag = "\"" + _this12.etag(req.parsed.href, body.length || 0, headers["last-modified"] || 0, body || 0) + "\"";
								}
			...
			fs.readdir(pages, function (e, files) {
				if (e) {
					...
					var next = function next(req, res) {
						_this14.pipeline(req, res).then(function (arg) {
							console.log("here");
							return arg;
						}, function (e) {
							return _this14.error(req, res, e);
						}).then(function () {
							_this14.log(_this14.prep(req, res, res._headers || {}), "info");
						});
					};
   ...
   */
    ...
    */
			var cleanup = function cleanup() {
				watcher.close();
				_this17.unregister(url);
				delete _this17.watching[path];
			};
					...
					if (regex.rename.test(ev)) {
						...
						fs.lstat(path, function (e, stat) {
							...
							if (e) {
								_this17.log(e);
								cleanup();
							} else if (_this17.etags.cache[url]) {
								value = _this17.etags.cache[url].value;
								value.etag = _this17.etag(url, stat.size, stat.mtime);
								value.timestamp = parseInt(new Date().getTime() / 1000, 10);
								_this17.register(url, value, true);
							} else {
								cleanup();
							}
						});
			...
			if (!put && regex.end_slash.test(req.url)) {
				...
				fs.lstat(path, function (e, stat) {
					...
					if (e) {
						...
						etag = "\"" + _this18.etag(req.parsed.href, stat.size, stat.mtime) + "\"";
						...
						if (!req.headers.hasOwnProperty("etag") || req.headers.etag === etag) {
							...
							deferred.resolve(_this18.respond(req, res, MESSAGES.NO_CONTENT, CODES.FAILED, {}, false));
						}
					}
				});
...
var factory = function factory() {
	...
	var etag = function etag(req, res, next) {
		var cached = undefined,
		    headers = undefined;
