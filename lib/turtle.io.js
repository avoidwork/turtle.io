...
var $           = require( "abaaso" ),
    ...
    REGEX_SLASH = /\/$/,
    REGEX_SPACE = /\s+/,
    REGEX_REWRITE = /^(text\/html|application\/json)/,
    MSG_ACK     = "acknowledge",
    MSG_ALL     = "announce",
    MSG_MASTER  = "master",
    MSG_READY   = "ready",
...
factory.prototype.start = function ( args, errorHandler ) {
	...
	if ( this.config.headers.Server === undefined ) {
		this.config.headers.Server = ( function () { return ( "turtle.io/0.9.5 (abaaso/" + $.version + " node.js/" + process.versions.node.replace( /^v/, "" ) + process.platform.capitalize() + " V8/" + process.versions.v8.toString().trim() + ")" ); } )();
	}
