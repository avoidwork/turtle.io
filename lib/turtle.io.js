...
var TurtleIO = (function () {
  ...
  */
		...
		value: function decorate(req, res) {
			...
			if (!this.allowed("get", req.parsed.pathname, req.vhost)) {
				this.get(req.parsed.pathname, function (req, res, next) {
					_this5.request(req, res).then(function () {
						next();
					}, function (e) {
						next(e);
					});
    ...
    */
			var handle = function handle(req, res, headers, status, arg) {
				...
				if (status >= CODES.BAD_REQUEST) {
					_this9.error(req, res, status, arg).then(function (arg) {
						deferred.resolve(arg);
					});
				} else {
					...
					if (status !== CODES.NOT_MODIFIED) {
						...
						if (get && status === CODES.SUCCESS) {
							etag = headers.etag || "\"" + _this9.etag(url, headers["content-length"] || 0, headers["last-modified"] || 0, _this9.encode(arg)) + "\"";
						...
						if (get && req.headers["if-none-match"] === etag) {
							...
							_this9.respond(req, res, MESSAGES.NO_CONTENT, CODES.NOT_MODIFIED, headers).then(function (arg) {
								deferred.resolve(arg);
							}, function (e) {
								deferred.reject(e);
							});
							...
							_this9.respond(req, res, arg, status, headers).then(function (arg) {
								deferred.resolve(arg);
							}, function (e) {
								deferred.reject(e);
							});
    ...
    */
			var wrapper = function wrapper(req, res) {
				...
				proxyReq.on("error", function (e) {
					_this9.error(req, res, regex.refused.test(e.message) ? CODES.SERVER_UNAVAILABLE : CODES.SERVER_ERROR, e.message);
				});
   ...
   * @return {Object}          TurtleIO instance
   */
		value: function respond(req, res, body, _x3, headers) {
   ...
   */
		value: function start(cfg, err) {
			...
			if (!this.config.headers.server) {
				this.config.headers.server = "turtle.io/4.0.1";
