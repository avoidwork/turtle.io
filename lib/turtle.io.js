					...
					if (!file) {
						...
						// Raw response body, compress and send to Client & disk
							...
							zlib[sMethod](body, function (e, data) {
								if (e) {
									_this.log(new Error("[client " + (req.headers["x-forwarded-for"] ? array.last(string.explode(req.headers["x-forwarded-for"])) : req.connection.remoteAddress) + "] " + e.message), "error");
									_this.unregister(req.parsed.href);
									_this.error(req, res, CODES.SERVER_ERROR);
								} else {
									if (!res._header && !res._headerSent) {
				...
				if (!dir) {
					if (regex.get.test(method)) {
						...
						fs.unlink(path, function (e) {
							if (e) {
								_this.error(req, req, CODES.SERVER_ERROR);
							} else {
								_this.respond(req, res, MESSAGES.NO_CONTENT, CODES.NO_CONTENT, {});
							}
						});
			...
			value: function request(req, res) {
				...
				fs.lstat(path, function (e, stats) {
					if (e) {
						...
						_this.error(req, res, CODES.NOT_FOUND);
					} else if (!stats.isDirectory()) {
						end();
						_this.handle(req, res, path, req.parsed.href, false, stats);
					} else if (regex.get.test(method) && !regex.dir.test(req.parsed.pathname)) {
						end();
						_this.respond(req, res, MESSAGES.NO_CONTENT, CODES.REDIRECT, { Location: (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + req.parsed.search });
					} else if (!regex.get.test(method)) {
						end();
						_this.handle(req, res, path, req.parsed.href, true);
					} else {
						count = 0;
						nth = _this.config.index.length;
						path += "/";

						array.iterate(_this.config.index, function (i) {
							fs.lstat(path + i, function (e, stats) {
								if (!e && !handled) {
									handled = true;
									end();
									_this.handle(req, res, path + i, (req.parsed.pathname != "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, stats);
				...
				if (regex.body.test(method)) {
					...
					req.on("data", function (data) {
						...
						if (_this.config.maxBytes > 0 && Buffer.byteLength(payload) > _this.config.maxBytes) {
							req.invalid = true;
							_this.error(req, res, CODES.REQ_TOO_LARGE);
						}
					});
					...
					req.on("end", function () {
						if (!req.invalid) {
							...
							_this.run(req, res, req.vhost, method);
						}
					});
				}
				// Running middleware
			...
			value: function run(req, res, host, method) {
				...
				var stop = function (timer) {
					if (timer.stopped === null) {
						timer.stop();
						_this.signal("middleware", function () {
    ...
    */
			...
			value: function start(cfg, err) {
				...
				if (!this.config.headers.server) {
					this.config.headers.server = "turtle.io/3.2.10";
				...
				fs.readdir(pages, function (e, files) {
					if (e) {
						...
						array.iterate(files, function (i) {
							_this.pages.all[i.replace(regex.next, "")] = fs.readFileSync(pages + "/" + i, "utf8");
						});

						// Starting server
						if (_this.server === null) {
							// For proxy behavior
							if (https.globalAgent.maxSockets < _this.config.proxy.maxConnections) {
								https.globalAgent.maxConnections = _this.config.proxy.maxConnections;
							}

							if (http.globalAgent.maxSockets < _this.config.proxy.maxConnections) {
								http.globalAgent.maxConnections = _this.config.proxy.maxConnections;
							}
							...
							if (_this.config.ssl.cert !== null && _this.config.ssl.key !== null) {
								...
								_this.server = https.createServer(merge(_this.config.ssl, {
									...
									_this.route(req, res);
								}).listen(_this.config.port, _this.config.address);
							} else {
				...
				if (!put && regex.end_slash.test(req.url)) {
					...
					fs.lstat(path, function (e, stat) {
						if (e) {
							...
							var etag = "\"" + _this.etag(req.parsed.href, stat.size, stat.mtime) + "\"";
							...
							if (!req.headers.hasOwnProperty("etag") || req.headers.etag === etag) {
								fs.writeFile(path, body, function (e) {
									if (e) {
										...
										_this.respond(req, res, _this.page(status, _this.hostname(req)), status, { allow: allow }, false);
									}
								});
							} else if (req.headers.etag !== etag) {
								_this.respond(req, res, MESSAGES.NO_CONTENT, CODES.FAILED, {}, false);
							}
						}
					});
