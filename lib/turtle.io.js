 ...
 * @param  {String}  iv     [Optional] Salt for encryption
		...
		try {
			...
			// Getting or creating an Etag
				...
				if ( xhr.status === self.codes.SUCCESS ) {
					etag = resHeaders.Etag || "\"" + self.etag( url, resHeaders["Content-Length"] || 0, resHeaders["Last-Modified"] || 0, typeof arg === "object" ? $.encode( arg ) : arg ) + "\"";
...
TurtleIO.prototype.start = function ( cfg, err ) {
	...
	if ( !this.config.headers.Server ) {
		this.config.headers.Server = "turtle.io/0.12.19 (abaaso/" + $.version + " node.js/" + process.versions.node.replace( /^v/, "" ) + process.platform.capitalize() + " V8/" + process.versions.v8.toString().trim() + ")";
	}
