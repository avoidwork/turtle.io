...
var TurtleIO = function () {
	...
	_createClass(TurtleIO, [{
		...
		value: function register(uri, state) {
			["content-length", "content-encoding", "server", "status", "transfer-encoding", "x-powered-by", "x-response-time", "access-control-allow-origin", "access-control-expose-headers", "access-control-max-age", "access-control-allow-credentials", "access-control-allow-methods", "access-control-allow-headers"].forEach(function (i) {
			...
			if (!res._header && !res._headerSent) {
				...
				if (compression) {
					...
					if (pipe) {
						...
						body.pipe(zlib[compressionMethod]()).on("error", errHandler).on("end", function () {
							deferred.resolve(true);
						}).pipe(res);
					} else {
						zlib[compressionMethod](body, function (e, data) {
							if (e) {
								errHandler(e);
							} else {
