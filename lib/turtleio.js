  ...
  */
					...
					if (typeof body.pipe === "function") {
						...
						zlib[sMethod](body, function (e, data) {
							if (e) {
								...
								deferred.reject(new Error(_this3.codes.INTERNAL_SERVER_ERROR));
							} else {
								if (!res._header && !res._headerSent) {
   ...
   */
		...
		value: function decorate(req, res) {
			...
			this.vhostsRegExp.forEach(function (i, idx) {
				if (i.test(hostname)) {
			...
			if (!dir) {
				...
				if (invalid) {
					...
					fs.unlink(fpath, function (e) {
						if (e) {
							deferred.reject(new Error(_this6.codes.INTERNAL_SERVER_ERROR));
						} else {
							_this6.respond(req, res, _this6.messages.NO_CONTENT, _this6.codes.NO_CONTENT, {}).then(deferred.resolve, deferred.reject);
						}
					});
   ...
   */
		...
		value: function page() {
			...
			var host = arguments.length <= 1 || arguments[1] === undefined ? all : arguments[1];

			var result = this.pages.find({ host: host, status: status });
   ...
   */
		...
		value: function register(uri, state, purge) {
			...
			["content-encoding", "server", "status", "transfer-encoding", "x-powered-by", "x-response-time", "access-control-allow-origin", "access-control-expose-headers", "access-control-max-age", "access-control-allow-credentials", "access-control-allow-methods", "access-control-allow-headers"].forEach(function (i) {
			...
			if (req.headers.expect) {
				...
				fs.lstat(lpath, function (e, stats) {
					if (e) {
						...
						_this10.handle(req, res, lpath, req.parsed.href, false, stats).then(deferred.resolve, deferred.reject);
					} else if (regex.get.test(method) && !regex.dir.test(req.parsed.pathname)) {
						end();
						_this10.respond(req, res, _this10.messages.NO_CONTENT, _this10.codes.TEMPORARY_REDIRECT, { "location": (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + req.parsed.search }).then(deferred.resolve, deferred.reject);
					} else if (!regex.get.test(method)) {
						end();
						_this10.handle(req, res, lpath, req.parsed.href, true).then(deferred.resolve, deferred.reject);
					} else {
						count = 0;
						nth = _this10.config.index.length;

						_this10.config.index.forEach(function (i) {
							...
							fs.lstat(npath, function (err, lstats) {
								if (!err && !handled) {
									...
									_this10.handle(req, res, npath, (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, lstats).then(deferred.resolve, deferred.reject);
   ...
   */
		...
		value: function respond(req, res, body) {
			...
			if (req.headers.range) {
				options = {};
				(req.headers.range.match(/\d+/g) || []).forEach(function (i, idx) {
			...
			if (ua && (lstatus === this.codes.OK || lstatus === this.codes.PARTIAL_CONTENT) && lbody !== this.messages.NO_CONTENT && this.config.compress && (type = this.compression(ua, encoding, lheaders["content-type"])) && !utility.isEmpty(type)) {
				...
				fs.createReadStream(lbody, options).on("error", function () {
					deferred.reject(new Error(_this11.codes.INTERNAL_SERVER_ERROR));
				}).on("close", function () {
					deferred.resolve(true);
				}).pipe(res);
			} else {
   ...
   */
		...
		value: function start() {
			...
			array.cast(config.vhosts, true).forEach(function (i) {
				_this12.host(i);
			});
			...
			fs.readdir(pages, function (e, files) {
				if (e) {
				...
				} else if (Object.keys(_this12.config).length > 0) {
					var pipe = function pipe(req, res) {
						_this12.decorate(req, res);
						router(req, res).then(function (arg) {
							...
							if (isNaN(errz.message)) {
								body = errz;
								status = new Error(_this12.codes.INTERNAL_SERVER_ERROR);
							} else {
								body = errz.extended;
								status = Number(errz.message);
							}

							return _this12.error(req, res, status, body);
						});
					};

					_this12.pages.batch(files.map(function (i) {
						return {
							host: all,
					...
					if (_this12.server === null) {
						if (_this12.config.ssl.cert !== null && _this12.config.ssl.key !== null) {
							...
							// Starting server
							_this12.server = https.createServer(utility.merge(_this12.config.ssl, {
							...
							}), pipe).listen(_this12.config.port, _this12.config.address);
						} else {
							_this12.server = http.createServer(pipe).listen(_this12.config.port, _this12.config.address);
						}
					} else {
   ...
   */
    ...
    */
			var cleanup = function cleanup() {
				_this15.watching[fpath].close();
				_this15.unregister(uri);
				delete _this15.watching[fpath];
			};
					...
					if (regex.rename.test(ev)) {
						...
						fs.lstat(fpath, function (e, stat) {
							...
							if (e) {
								_this15.log(e);
								cleanup();
							} else if (_this15.etags.cache[uri]) {
								value = _this15.etags.cache[uri].value;
								value.etag = _this15.etag(uri, stat.size, stat.mtime).toString();
								value.headers.etag = "\"" + value.etag + "\"";
								value.timestamp = parseInt(new Date().getTime() / 1000, 10);
								_this15.register(uri, value, true);
							} else {
								cleanup();
							}
						});
		...
		value: function write(req, res, fpath) {
			...
			if (!put && regex.end_slash.test(req.url)) {
				...
				fs.lstat(fpath, function (e, stat) {
					...
					if (e) {
						deferred.reject(new Error(_this16.codes.NOT_FOUND));
					} else {
						letag = "\"" + _this16.etag(req.parsed.href, stat.size, stat.mtime) + "\"";

						if (!req.headers.hasOwnProperty("etag") || req.headers.etag === letag) {
							fs.writeFile(fpath, body, function (err) {
								if (err) {
									deferred.reject(new Error(_this16.codes.INTERNAL_SERVER_ERROR));
								} else {
									status = _this16.codes[put ? "NO_CONTENT" : "CREATED"];
									deferred.resolve(_this16.respond(req, res, _this16.page(status, _this16.hostname(req)), status, { allow: allow }, false));
								}
							});
						} else if (req.headers.etag !== letag) {
							deferred.resolve(_this16.respond(req, res, _this16.messages.NO_CONTENT, _this16.codes.PRECONDITION_FAILED, {}, false));
						}
					}
				});
