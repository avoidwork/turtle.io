		...
		value: function route(req, res) {
			...
			var last = function last(err) {
				...
				if (!err) {
					...
					status = res.statusCode >= _this12.codes.BAD_REQUEST ? res.statusCode : !isNaN(err.message) ? err.message : _this12.codes[(err.message || err).toUpperCase()] || _this12.codes.SERVER_ERROR;
				...
				if (e) {
					...
					if (_this13.server === null) {
						if (_this13.config.ssl.cert !== null && _this13.config.ssl.key !== null) {
							...
							// Starting server
							_this13.server = https.createServer(merge(_this13.config.ssl, {
