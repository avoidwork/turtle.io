			...
			if (req.headers.expect) {
				...
				fs.lstat(lpath, function (e, stats) {
					if (e) {
						...
						_this10.respond(req, res, _this10.messages.NO_CONTENT, _this10.codes.TEMPORARY_REDIRECT, { "location": (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + req.parsed.search }).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
				...
				if (e) {
					...
					if (_this12.server === null) {
						if (_this12.config.ssl.cert !== null && _this12.config.ssl.key !== null) {
							...
							}), next).listen(_this12.config.port, _this12.config.address);
						} else {
							_this12.server = http.createServer(next).listen(_this12.config.port, _this12.config.address);
						}
					} else {
