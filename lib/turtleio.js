					...
					var next = function next(req, res) {
						...
						router(req, res).then(function (arg) {
							...
							if (isNaN(errz.message)) {
								body = errz;
								status = new Error(_this12.codes.SERVER_ERROR);
							} else {
								body = errz.extended;
					...
					if (e) {
						...
						if (!req.headers.hasOwnProperty("etag") || req.headers.etag === letag) {
							fs.writeFile(fpath, body, function (err) {
								if (err) {
									deferred.reject(new Error(_this16.codes.SERVER_ERROR));
								} else {
							...
							deferred.resolve(_this16.respond(req, res, _this16.messages.NO_CONTENT, _this16.codes.FAILED, {}, false));
						}
					}
				});
