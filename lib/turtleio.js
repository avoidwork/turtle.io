...
class TurtleIO {
	...
	etag (...args) {
		return this.config.etags.ignore.filter(i => i === args[0]).length === 0 ? this.etags.create(args.join("-")) : "";
	}
	...
	headers (req, res, status, body, headers, pipe) {
		...
		if (!req.cors) {
			...
			result["access-control-allow-headers"] = req.headers["access-control-allow-headers"] || "accept, content-type";
		}
	...
	stop () {
		if (!this.server) {
			...
			// Clearing watchers
			this.watching.forEach((key, watcher) => {
				watcher.close();
			});
