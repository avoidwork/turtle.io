  ...
  */
   ...
   */
		...
		value: function headers(req) {
			...
			var status = arguments.length <= 2 || arguments[2] === undefined ? codes.SUCCESS : arguments[2];
    ...
    */
			var handle = function handle(req, res, headers, status, arg) {
				...
				if (status >= _this8.codes.BAD_REQUEST) {
					...
					if (get && (status === _this8.codes.SUCCESS || status === _this8.codes.NOT_MODIFIED) && !regex.nocache.test(headers["cache-control"]) && !regex["private"].test(headers["cache-control"])) {
					...
					if (status !== _this8.codes.NOT_MODIFIED) {
						...
						// Setting headers
						if (get && status === _this8.codes.SUCCESS) {
   ...
   */
		...
		value: function respond(req, res, body) {
			var status = arguments.length <= 3 || arguments[3] === undefined ? codes.SUCCESS : arguments[3];
			...
			var finalize = function finalize() {
				...
				if (regex.get_only.test(req.method) && (lstatus === _this11.codes.SUCCESS || lstatus === _this11.codes.NOT_MODIFIED)) {
					// Updating cache
   ...
   */
		...
		value: function route(req, res) {
			...
			var last = function last(err) {
				var errorCode = !isNaN(err.message) ? err.message : _this12.codes[(err.message || err).toUpperCase()] || _this12.codes.SERVER_ERROR,
				    error = undefined,
				    status = undefined;
