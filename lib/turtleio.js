  ...
  */
	...
	_createClass(TurtleIO, [{
		...
		value: function allowed(method, uri, host, override) {
			...
			result = this.routes(uri, host, method, override).filter(function (i) {
   ...
   */
		...
		key: "compress",
		value: function compress(req, res, body, type, letag, file, options, status, headers) {
			var _this3 = this;
					...
					if (typeof body.pipe === "function") {
						...
						zlib[sMethod](body, function (e, data) {
							if (e) {
								...
								deferred.reject(new Error(_this3.codes.SERVER_ERROR));
							} else {
								if (!res._header && !res._headerSent) {
   ...
   */
		...
		value: function decorate(req, res) {
			...
			array.each(this.vhostsRegExp, function (i, idx) {
				if (i.test(hostname)) {
					return !(req.vhost = _this4.vhosts[idx]);
				}
			});
			...
			if (!this.allowed("get", req.parsed.pathname, req.vhost)) {
				this.get(req.parsed.pathname, function (req2, res2, next2) {
					_this4.request(req2, res2).then(function () {
						next2();
					}, function (e) {
						next2(e);
					});
			...
			res.redirect = function (target) {
				return _this4.respond.call(_this4, req, res, _this4.messages.NO_CONTENT, _this4.codes.FOUND, { location: target });
			};

			res.respond = function (arg, status, headers) {
				return _this4.respond.call(_this4, req, res, arg, status, headers);
			};

			res.error = function (status, arg) {
				return _this4.error.call(_this4, req, res, status, arg);
			};
		}
   ...
   */
		...
		value: function error(req, res, status, msg) {
			...
			this.signal("error", function () {
				return [req.vhost, req.parsed.path, lstatus, msg || kdx ? array.cast(_this5.messages)[kdx] : "Unknown error", timer.diff()];
			});
			...
			if (!dir) {
				...
				if (invalid) {
					...
					fs.unlink(fpath, function (e) {
						if (e) {
							deferred.reject(new Error(_this6.codes.SERVER_ERROR));
						} else {
							_this6.respond(req, res, _this6.messages.NO_CONTENT, _this6.codes.NO_CONTENT, {}).then(function (arg) {
								deferred.resolve(arg);
							}, function (err) {
								deferred.reject(err);
							});
   ...
   */
		...
		value: function headers(req) {
			var lRHeaders = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
			var status = arguments.length <= 2 || arguments[2] === undefined ? codes.OK : arguments[2];
   ...
   */
		...
		value: function log(arg) {
			...
			var level = arguments.length <= 1 || arguments[1] === undefined ? "notice" : arguments[1];
			...
			if (logging) {
				...
				this.signal("log", function () {
					return [level, _this7.config.logs.stdout, false, timer.diff()];
				});
			}
   ...
   */
		...
		value: function proxy(route, origin, host) {
			...
			var stream = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
    ...
    */
			var handle = function handle(req, res, headers, status, arg) {
				...
				headers.server = _this8.config.headers.server;

				if (status >= _this8.codes.BAD_REQUEST) {
					_this8.error(req, res, status, arg).then(function (argz) {
						deferred.resolve(argz);
					});
				} else {
					// Determining if the response will be cached
					if (get && (status === _this8.codes.OK || status === _this8.codes.NOT_MODIFIED) && !regex.nocache.test(headers["cache-control"]) && !regex["private"].test(headers["cache-control"])) {
						...
						if (lstale > 0) {
							setTimeout(function () {
								_this8.unregister(uri);
							}, lstale * 1000);
						}
					}

					if (status !== _this8.codes.NOT_MODIFIED) {
						rewrite = regex.rewrite.test((headers["content-type"] || "").replace(regex.nval, ""));

						// Setting headers
						if (get && status === _this8.codes.OK) {
							letag = headers.etag || "\"" + _this8.etag(uri, headers["content-length"] || 0, headers["last-modified"] || 0, _this8.encode(arg)) + "\"";
						...
						if (get && req.headers["if-none-match"] === letag) {
							cached = _this8.etags.get(uri);

							if (cached) {
								headers.age = parseInt(new Date().getTime() / 1000 - cached.value.timestamp, 10);
							}

							_this8.respond(req, res, _this8.messages.NO_CONTENT, _this8.codes.NOT_MODIFIED, headers).then(function (argz) {
								deferred.resolve(argz);
							}, function (e) {
								deferred.reject(e);
							});
							...
							if (regex.head.test(req.method.toLowerCase())) {
								larg = _this8.messages.NO_CONTENT;
							} else if (rewrite) {
							...
							_this8.respond(req, res, larg, status, headers).then(function (argz) {
								deferred.resolve(argz);
							}, function (e) {
								deferred.reject(e);
							});
						...
						_this8.respond(req, res, arg, status, headers).then(function (argz) {
							deferred.resolve(argz);
						}, function (e) {
							deferred.reject(e);
						});
    ...
    */
			var wrapper = function wrapper(req, res) {
				...
				headerz["x-forwarded-server"] = _this8.config.headers.server;
				...
				proxyReq.on("error", function (e) {
					_this8.error(req, res, _this8.codes[regex.refused.test(e.message) ? "SERVER_UNAVAILABLE" : "SERVER_ERROR"], e.message);
				});
			...
			array.each(verbs, function (i) {
				if (route === "/") {
					_this8[i]("/.*", wrapper, host);
				} else {
					_this8[i](route, wrapper, host);
					_this8[i](route + "/.*", wrapper, host);
				}
			});
   ...
   */
		...
		value: function redirect(route, uri, host) {
			...
			var permanent = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
			...
			this.get(route, function (req, res) {
				...
				_this9.respond(req, res, _this9.messages.NO_CONTENT, _this9.codes[permanent ? "MOVED" : "REDIRECT"], {
   ...
   */
		...
		key: "register",
		value: function register(uri, state, purge) {
   ...
   */
		...
		value: function request(req, res) {
			...
			var end = function end() {
				timer.stop();
				_this10.signal("request", function () {
			...
			if (req.headers.expect) {
				...
				fs.lstat(lpath, function (e, stats) {
					if (e) {
						...
						deferred.reject(new Error(_this10.codes.NOT_FOUND));
					} else if (!stats.isDirectory()) {
						end();
						_this10.handle(req, res, lpath, req.parsed.href, false, stats).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						_this10.respond(req, res, _this10.messages.NO_CONTENT, _this10.codes.REDIRECT, { "location": (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + req.parsed.search }).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						_this10.handle(req, res, lpath, req.parsed.href, true).then(function (arg) {
							deferred.resolve(arg);
						}, function (err) {
							deferred.reject(err);
						});
						...
						count = 0;
						nth = _this10.config.index.length;

						array.each(_this10.config.index, function (i) {
							...
							fs.lstat(npath, function (err, lstats) {
								if (!err && !handled) {
									...
									_this10.handle(req, res, npath, (req.parsed.pathname !== "/" ? req.parsed.pathname : "") + "/" + i + req.parsed.search, false, lstats).then(function (arg) {
										deferred.resolve(arg);
									}, function (errz) {
   ...
   */
		...
		value: function respond(req, res, body) {
			var status = arguments.length <= 3 || arguments[3] === undefined ? codes.OK : arguments[3];

			var _this11 = this;

			var headers = arguments.length <= 4 || arguments[4] === undefined ? { "content-type": "text/plain" } : arguments[4];
			var file = arguments.length <= 5 || arguments[5] === undefined ? false : arguments[5];
			...
			var finalize = function finalize() {
				...
				if (regex.get_only.test(req.method) && (lstatus === _this11.codes.OK || lstatus === _this11.codes.NOT_MODIFIED)) {
					// Updating cache
					if (!regex.nocache.test(lheaders["cache-control"]) && !regex["private"].test(lheaders["cache-control"])) {
						cached = _this11.etags.get(req.parsed.href);

						if (!cached) {
							if (lheaders.etag === undefined) {
								lheaders.etag = "\"" + _this11.etag(req.parsed.href, lbody.length || 0, lheaders["last-modified"] || 0, lbody || 0) + "\"";
							}
							...
							_this11.register(req.parsed.href, {
					...
					if (req.path) {
						_this11.watch(req.parsed.href, req.path);
					}
				} else if (lstatus === _this11.codes.NOT_FOUND) {
			...
			if (ua && (lstatus === this.codes.OK || lstatus === this.codes.PARTIAL_CONTENT) && lbody !== this.messages.NO_CONTENT && this.config.compress && (type = this.compression(ua, encoding, lheaders["content-type"])) && !utility.isEmpty(type)) {
				...
				fs.createReadStream(lbody, options).on("error", function () {
					deferred.reject(new Error(_this11.codes.SERVER_ERROR));
				}).on("close", function () {
					deferred.resolve(true);
				}).pipe(res);
			} else {
			...
			process.nextTick(function () {
				_this11.log(_this11.prep(req, res, lheaders), "info");
			});
		...
		value: function route(req, res) {
			...
			var last = function last(err) {
				...
				if (!err) {
					...
					} else if (_this12.allowed("get", req.parsed.pathname, req.vhost)) {
						deferred.reject(new Error(_this12.codes.NOT_ALLOWED));
					} else {
						deferred.reject(new Error(_this12.codes.NOT_FOUND));
					}
				} else {
					errorCode = !isNaN(err.message) ? err.message : _this12.codes[(err.message || err).toUpperCase()] || _this12.codes.SERVER_ERROR;
					status = res.statusCode >= _this12.codes.BAD_REQUEST ? res.statusCode : errorCode;
			...
			var next = function next(err) {
				...
				} else if (!res._header && _this12.config.catchAll) {
					last(err);
				} else if (res._header) {
					deferred.resolve([req, res]);
				}
   ...
   */
		...
		value: function start() {
			...
			var err = arguments.length <= 1 || arguments[1] === undefined ? undefined : arguments[1];

			var config = utility.merge(utility.clone(defaultConfig), cfg),
			    headers = undefined,
			    pages = undefined;
			...
			utility.iterate(headers, function (value, key) {
				_this13.config.headers[key.toLowerCase()] = value;
			});
			...
			fs.readdir(pages, function (e, files) {
				if (e) {
					_this13.log(new Error("[client 0.0.0.0] " + e.message), "error");
				} else if (array.keys(_this13.config).length > 0) {
					var next = function next(req, res) {
						_this13.route(req, res).then(function (arg) {
							return arg;
						}, function (errz) {
							var body = undefined,
							    status = undefined;
							...
							if (isNaN(errz.message)) {
								body = errz;
								status = new Error(_this13.codes.SERVER_ERROR);
							} else {
								body = errz.extended;
								status = Number(errz.message);
							}

							return _this13.error(req, res, status, body);
						});
					};

					array.each(files, function (i) {
						_this13.pages.all[i.replace(regex.next, "")] = fs.readFileSync(path.join(pages, i), "utf8");
					});

					// Starting server
					if (_this13.server === null) {
						if (_this13.config.ssl.cert !== null && _this13.config.ssl.key !== null) {
							// POODLE
							_this13.config.secureProtocol = "SSLv23_method";
							_this13.config.secureOptions = constants.SSL_OP_NO_SSLv3 | constants.SSL_OP_NO_SSLv2;

							// Reading files
							_this13.config.ssl.cert = fs.readFileSync(_this13.config.ssl.cert);
							_this13.config.ssl.key = fs.readFileSync(_this13.config.ssl.key);

							// Starting server
							_this13.server = https.createServer(utility.merge(_this13.config.ssl, {
								port: _this13.config.port,
								host: _this13.config.address,
								secureProtocol: _this13.config.secureProtocol,
								secureOptions: _this13.config.secureOptions
							}), next).listen(_this13.config.port, _this13.config.address);
						} else {
							_this13.server = http.createServer(next).listen(_this13.config.port, _this13.config.address);
						}
					} else {
						_this13.server.listen(_this13.config.port, _this13.config.address);
					}

					// Dropping process
					if (_this13.config.uid && !isNaN(_this13.config.uid)) {
						process.setuid(_this13.config.uid);
					}

					_this13.log("Started " + _this13.config.id + " on port " + _this13.config.port, "debug");
				}
			});
   ...
   */
    ...
    */
			var cleanup = function cleanup() {
				_this16.watching[fpath].close();
				_this16.unregister(uri);
				delete _this16.watching[fpath];
			};

			if (this.watching[fpath] === undefined) {
				this.watching[fpath] = fs.watch(fpath, function (ev) {
					if (regex.rename.test(ev)) {
						...
						fs.lstat(fpath, function (e, stat) {
							...
							if (e) {
								_this16.log(e);
								cleanup();
							} else if (_this16.etags.cache[uri]) {
								value = _this16.etags.cache[uri].value;
								value.etag = _this16.etag(uri, stat.size, stat.mtime).toString();
								value.headers.etag = "\"" + value.etag + "\"";
								value.timestamp = parseInt(new Date().getTime() / 1000, 10);
								_this16.register(uri, value, true);
							} else {
								cleanup();
							}
						});
		...
		value: function write(req, res, fpath) {
			...
			if (!put && regex.end_slash.test(req.url)) {
				...
				fs.lstat(fpath, function (e, stat) {
					...
					if (e) {
						deferred.reject(new Error(_this17.codes.NOT_FOUND));
					} else {
						letag = "\"" + _this17.etag(req.parsed.href, stat.size, stat.mtime) + "\"";

						if (!req.headers.hasOwnProperty("etag") || req.headers.etag === letag) {
							fs.writeFile(fpath, body, function (err) {
								if (err) {
									deferred.reject(new Error(_this17.codes.SERVER_ERROR));
								} else {
									status = _this17.codes[put ? "NO_CONTENT" : "CREATED"];
									deferred.resolve(_this17.respond(req, res, _this17.page(status, _this17.hostname(req)), status, { allow: allow }, false));
								}
							});
						} else if (req.headers.etag !== letag) {
							deferred.resolve(_this17.respond(req, res, _this17.messages.NO_CONTENT, _this17.codes.FAILED, {}, false));
						}
					}
				});
