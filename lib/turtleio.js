...
class TurtleIO {
	...
	canETag (pathname, method, headers = this.config.headers) {
		return method === "GET" && this.etags.valid(headers) && !this.config.etags.invalid.filter(i => i.test(pathname)).length;
	}
	...
	send (req, res, body = "", status = 200, headers = {"content-type": "text/plain"}) {
		...
		let header, lheaders, compression, compressionMethod;
	...
	"static" (req, res) {
		middleware.file(req, res, err => {
			if (err) {
				...
				middleware.stream(req, res, err2 => res.error(err2));
			}
		});
	}
